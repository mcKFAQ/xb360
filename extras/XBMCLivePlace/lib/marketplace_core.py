...
URL = 'http://www.tintense.com/rss.php'
...
def getScriptlist_site(callback):
    printDebug(msg, "Fetching content list from www.tintense.com")
...
class ParseXML:
    ...
    def parsesite(self, callback):
        ...
        if doc != None:
            scriptlist = []
            channel = doc.firstChild.childNodes
            for item in channel:
                if item.nodeType != xml.dom.minidom.Node.ELEMENT_NODE: continue
                script_temp = {}
                description = category = location = date = downloads = "Unknown"
...
def DownloaderClass(message,url,dest):
    ...
    dp.create("Downloading",message,url)
